    ...
    <div className='flex flex-col justify-between w-[600px] p-2  rounded-md	my-2'>
      ...
      <div className='flex flex-col items-center '>
        ...
        {localStorage.getItem('userId') && <div className='w-full flex justify-between items-center'>
          ...
          <Link className='my-4 w-fit h-[3rem] w-[7rem] text-lg px-2' to='/myPage'>Your profile</Link>
          
        </div>}
        {localStorage.getItem('userId') &&<div className='w-full flex justify-between items-center'>
          <p>Watch your friends wishes</p>
          <Link className='my-4 w-fit h-[3rem] w-[7rem] text-lg px-2' to='/friends'>Friends</Link>
        </div>}
        {localStorage.getItem('userId') &&<div className='w-full flex justify-between items-center'>
          <p>Find friends </p>
          <Link className='my-4 w-fit h-[3rem] w-[7rem] text-lg px-2' to='/findUser'>Find friends</Link>
        </div>}
        {!localStorage.getItem('userId') &&<div className='w-full flex justify-between items-center'>
          <p>Log-In or Sign-Up</p>
          <Link className='my-4 w-fit h-[3rem] w-[7rem] text-lg px-2' to='/login'>Log-in</Link>
        </div> }
